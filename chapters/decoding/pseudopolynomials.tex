...
\begin{Definition}{Pseudodegree}{pseudodegree}
  ...
  \]
  We say $F(X)$ is a \emph{$k$-pseudopolynomial} if $\pdeg(F) \leq k$. When $\deg(F) = d$ we refer to $F$ as a \emph{$k$-pseudopolynomial of degree $d$}.
\end{Definition}
...
\begin{lemma}{Multiplicity Bound for Pseudopolynomials}{pdeg-mult-bound}
...
\begin{lemma}{}{lem:twisted-two}
  Let $c, h, M$ be parameters, with $c < M/2$. Let $r : \bbF_q \to \bbF_q$. Suppose $F(X), G(X) \in \bbF_q[X]$ with $\deg(F), \deg(G) < cq$ are both $r$-twisted $(h, M)$-pseudopolynomials and $k > \nicefrac{M}{M - 2c} \cdot (h + 1)$. Then there exist nonzero $k$-pseudopolynomials $A(X), B(X)$, with $\deg(A), \deg(B) < Mq$, such that:
  \[
...
\begin{lemma}{}{twisted-many}
  Let $m, c, h, M$ be parameters, with $c < \nicefrac{m-1}{m}M$. Let $r : \bbF_q \to \bbF_q$.

  Suppose $F_1(X), \ldots, F_m(X) \in \bbF_q[X]$ with $\deg(F_i) < cq$ are all $r$-twisted $(h, M)$-pseudopolynomials. Let $k > \nicefrac{M}{(m-1)M - mc} \cdot (h + 1)$. Then there exist $k$-pseudopolynomials $A_1(X), \ldots, A_m(X)$ with $\deg(A_i) < Mq$ and not all zero, such that:
  \[
    \sum_{i} A_i(X)\, F_i(X) = 0.
  \]
\end{lemma}
...
\begin{Theorem}{Robust Version for Many}{twisted-robust-many}
  Let $m, c, h, \gamma, M$ be parameters, with $c < \nicefrac{(m-1) - m\gamma}{m}M$. Let $r_1, \ldots, r_m : \bbF_q \to \bbF_q$, with:
  \[
    \bigl|\{\alpha \in \bbF_q \mid r_i(\alpha) \neq r_j(\alpha) \text{ for some } i, j\}\bigr| \leq \gamma q.
  \]
  Suppose $F_1(X), \ldots, F_m(X) \in \bbF_q[X]$ with $\deg(F_i) < cq$ are $r_i$-twisted $(h, M)$-pseudopolynomials. Let $k > \nicefrac{M}{(m - 1 - m\gamma)M - mc} \cdot (h + 1)$. Then there exist $k$-pseudopolynomials $A_1(X), \ldots, A_m(X)$, with $\deg(A_i) < Mq$, not all zero, such that:
  \[
    \sum_{i} A_i(X)\, F_i(X) = 0.
  \]
\end{Theorem}
