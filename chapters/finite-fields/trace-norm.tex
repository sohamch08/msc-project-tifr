...
Let $V$ be an $n$-dimensional vector space over $K$ where $K$ is a field.
Let $\vph:V\to V$ be any linear operator.
...
Note that here we defined the characteristic polynomial to be of the form $\det(X\cdot I_n-A)$ instead of $\det(A-X\cdot I_n)$ which we typically do in linear algebra. This is because we want our polynomials to monic.
...
\begin{corolary}{Formula of Trace and Norm in Finite Fields}{cor:trace-norm-fq-frobenius}
  Let $K/\bbF_q$ be finite extension with $[K\colon \bbF_q]=n$. Then for any $\alpha\in K$, $$\tr_{K/\bbF_q}(\alpha)=\alpha+\alpha^q+\cdots+\alpha^{q^{n-1}}\quad\text{and}\quad \norm_{K/\bbF_q}(\alpha)=\alpha\cdot \alpha^q\cdots\alpha^{q^{n-1}}$$
\end{corolary}
...
\begin{Theorem}{Transitive Property of Trace and Norm}{thm:trace-norm-transitivity}
