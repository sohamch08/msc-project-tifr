...
\begin{corolary}{}{}
  Let $f\in\bbF_q[X]$ be a polynomial with $\deg(g)\leq d$. Then the $\ell^{th}$ derivative of $g^j(X)$ is of the form $$(g^j)^{(\ell)}(X)=G_{g,j,\ell}(X)\cdot g(X)^{j-\ell}$$where $G_{g,j,\ell}(X)\in\bbF_q[X]$ and $\deg(G_{g,j,\ell})\leq d\ell-\ell$. 
\end{corolary}
