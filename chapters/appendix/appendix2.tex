...
\begin{lemma}{}{}
  \begin{enumerate}[label=(\roman*)]
    \item For $f_1,\dots, f_t\in\bbF[X]$ then we have $$(f_1\cdot f_t)^{(n)}=\sum_{\substack{n_1,\dots, n_t\geq 0\\ n_1+\cdots n_t=n}}\prod_{i=1}^tf_i^{(n_i)}$$ 
    \item For any $\alpha\in\bbF$ we have $$\lt((X-c)^t\rt)^{(n)}=\binom{t}n\cdot (X-c)^{t-n}.$$  
    \item For $0\leq n\leq $ and $f,g\in \bbF[X]$we have $$\lt(gf^t\rt)^{(n)}=\tdg\cdot f^{t-n}$$ where $\tdg\in\bbF[X]$ with $\deg(\tdg)\leq \deg(g)+n(\deg(f)-1)$.  
  \end{enumerate}
\end{lemma}
