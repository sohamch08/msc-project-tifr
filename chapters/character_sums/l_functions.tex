...
The characters of cyclic groups defined in \lemref{lem:chars-cyclic-group} are precisely the classical \emph{Dirichlet characters}.\index{Dirichlet, Peter}\index{L-function} Dirichlet characters are of paramount importance in analytic number theory: they appear in the study of primes in arithmetic progressions, the distribution of prime ideals, and the analytic continuation of $L$-functions. In the finite-field setting, the multiplicative characters of $\bbF_q$ play an analogous role, and the associated $L$-functions encode deep arithmetic information about character sums over $\bbF_q$. The treatment here focuses on the aspects directly relevant to the Weil bounds; readers interested in the general theory of $L$-functions in its full depth are encouraged to consult Iwaniec and Kowalski \cite{Iwaniec2012} for the analytic theory, and Miller and Takloo-Bighash \cite{Miller2006} for a broader introduction to modern number theory and the role of $L$-functions within it.
...
\begin{Definition}{Riemann Zeta Function}{}
  ...
  For any complex number $s\in\bbC$, the zeta function at $s$ is $$\zeta(s)=\sum_{n=1}^{\infty}\frac{1}{n^s}$$
\end{Definition}
...
\begin{Theorem}{Euler Product for $\zeta(s)$}{euler-product-classical}%
...
\end{Theorem}
regarding the zeros of the zeta function. The line $\Re(s)=1/2$ is the \emph{critical line}\index{critical line}, and the most famous open problem in mathematics is:
...
\begin{Theorem}{Euler Product for $\zeta(s)$}{thm:l-euler-product}%
  ...
  For $s\in\bbC$ with $\Re(s)>1$ we have $$L(s,\Lambda)=\prod_{\substack{h\in\Phi\\ h\text{ irreducible}}}(1-\Lambda(h)\cdot \mfn(h)^{-s})^{-1}$$
\end{Theorem}
\begin{proof}
  Since $\Re(s)>1$ we have \begin{align*}
    L(s,\Lambda) & = \sum_{h\in\Phi}\Lambda(h)\cdot \mfn(h)^{-s} = \prod_{\substack{h\in\Phi\\ h\text{ irreducible}}} \lt(\sum_{i=0}^{\infty} \Lambda(h^i)\cdot \mfn(h^i)^{-s} \rt) = \prod_{\substack{h\in\Phi\\ h\text{ irreducible}}}(1-\Lambda(h)\cdot \mfn(h)^{-s})^{-1}
  \end{align*}Hence we have the lemma.
\end{proof}
So let $\Lambda:G\to\bbC$ be a multiplicative complex valued function over the $G$ where $G$ is as defined above which also satisfies $|\Lambda(r)|\leq 1$ for all $r\in G$ and $\Lambda(1)=1$. Then for any $s\in\bbC$ with $\Re(s)>1$ we can rewrite $$L(s,\Lambda)=\sum_{h\in\Phi}\Lambda(h)\cdot \mfn(h)^{-s}=\sum_{d=0}^{\infty}\lt(\sum_{h\in\Phi_d}\Lambda(h)\rt)q^{-d\cdot s}$$
Since for any $h\in\Phi$ we have $\mfn(q)^s=\lt(q^s\rt)^{\deg(h)}$ we can consider the corresponding power series $$\ovL(z,\Lambda)=\sum_{h\in\Phi}\Lambda(h)\cdot z^{\deg(h)}$$hence $\ovL(q^{-s},\Lambda)=L(s,\Lambda)$. Now using \thmref{thm:l-euler-product} for $\ovL(z,\Lambda)$ we have $$\ovL(z,\Lambda)=\prod_{\substack{h\in\Phi\\ h\text{ irreducible}}}\lt(1-\Lambda(h)\cdot z^{\deg(h)}\rt)^{-1}$$Now we do some operations on the power series $\ovL(z,\Lambda)$ to get some expressions which help us later in proving some properties of exponential character sums. So for brevity of notations we use $\irr(\Phi)$ to denote the set of irreducible polynomials of $\Phi$. We have $\log \ovL(z,\Lambda)=-\sum\limits_{h\in\irr(\Phi)}\log \lt(1-\Lambda(h)z^{\deg(h)}\rt)$. So by differentiating we get $$\frac{d}{dz}\log \ovL(z,\Lambda) =-\sum\limits_{h\in\irr(\Phi)}\frac{-\Lambda(h)\cdot \deg(h)\cdot z^{\deg(h)-1}}{1-\Lambda(h)\cdot z^{\deg(h)}}=\sum\limits_{h\in\irr(\Phi)}\frac{\Lambda(h)\cdot \deg(h)\cdot z^{\deg(h)-1}}{1-\Lambda(h)\cdot z^{\deg(h)}}$$So \begin{align}
  z\cdot \frac{d}{dz}\log \ovL(z,\Lambda) & = \sum\limits_{h\in\irr(\Phi)}\frac{\Lambda(h)\cdot \deg(h)\cdot z^{\deg(h)-1}}{1-\Lambda(h)\cdot z^{\deg(h)}}\notag\\
  & = \sum_{h\in\irr(\Phi)}\Lambda(h)\cdot \deg(h)\cdot z^{\deg(h)}\sum_{k=0}^{\infty}\Lambda^k(h)\cdot z^{k\cdot \deg(h)}\notag\\
  & = \sum_{h\in\irr(\Phi)}\deg(h)\sum_{k=1}^{\infty}\Lambda^k(h)\cdot z^{k\cdot \deg(h)}\notag\\
  & = \sum_{k=1}^{\infty}\lt(\sum_{\substack{h\in\irr(\Phi),\\[0.5mm] \deg(h)\mid k}}\deg(h)\cdot \Lambda(h)^{\nicefrac{k}{\deg(h)}}\rt)z^k \label{eq:l-log-deriv-irreducibles}
\end{align}
So define $L_k\coloneqq  \sum_{h\in\irr(\Phi),\ \deg(h)\mid k}\deg(h)\cdot \Lambda(h)^{\nicefrac{k}{\deg(h)}}$. Hence we have $z\cdot \frac{d}{dz}\log \ovL(z,\Lambda)=\sum_{k=1}^{\infty}L_k\cdot z^k$. 

Now suppose there exists a positive integer $N\in\bbN$ such that for all $d>N$ we have $\sum_{h\in\Phi_d}\Lambda(g)=0$. Then we have $$L(s,\Lambda)=\sum_{d=0}^N \lt(\sum_{h\in\Phi_d}\Lambda(d)\rt)q^{-d\cdot s},\qquad  \ovL(z,\Lambda)=\sum_{d=0}^N \lt(\sum_{h\in\Phi_d}\Lambda(d)\rt)z^d$$
Since now $\ovL(z,\Lambda)$ is not an infinite power series anymore, instead a degree $N$ polynomial over $\bbC$ there exists complex numbers $w_1,\cdots, w_N\in\bbC$\index{reciprocal roots} such that $\ovL(z,\Lambda)=(1-w_1 z)(1-w_2 z)\cdots (1-w_N z)$.\index{L-function!finite degree} Now applying $z\cdot \frac{d}{dz}\log \ovL(z,\Lambda)$ on this expression we get \begin{align}
  z\cdot \frac{d}{dz}\log \ovL(z,\Lambda) & = -\sum_{i=1}^N\frac{w_iz}{1-w_iz}\notag\\
...
\begin{Theorem}{}{thm:l-function-root-factorisation}%
  ...
  Let $\Lambda:G\to\bbC$ be a multiplicative function with $|\Lambda(r)|\leq 1$ for all $r\in G$ and $\Lambda(1)=1$, and suppose there exists a positive integer $N\in\bbN$ such that $\sum_{h\in\Phi_d}\Lambda(h)=0$ for every $d>N$. Then $\ovL(z,\Lambda)$ is a polynomial of degree at most $N$ over $\bbC$, and there exist complex numbers $w_1,\dots,w_N\in\bbC$ such that
  $$\ovL(z,\Lambda)=\prod_{i=1}^{N}(1-w_iz), \qquad\text{ and }\qquad L(s,\Lambda)=\prod_{i=1}^{N}\lt(1-w_iq^{-s}\rt).$$
  Moreover, for every $k\in\bbN$,
  $$L_k\;=\;\sum_{\substack{h\in\irr(\Phi)\\ \deg(h)\mid k}}\deg(h)\cdot \Lambda(h)^{\nicefrac{k}{\deg(h)}}\;=\;-\sum_{i=1}^{N}w_i^k.$$
\end{Theorem}