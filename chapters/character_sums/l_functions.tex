...
\end{Theorem}
regarding the zeros of the zeta functionThe line $\Re(s)=1/2$ is the \emph{critical line}, and the most famous open problem in mathematics is:
...
where $\Lambda(n)=\log p$ if $n=p^k$ and $0$ otherwise (the \emph{von Mangoldt function}). Therefore we can see how the non-trivial zeros of $\zeta(s)$ and the primes are very related.
