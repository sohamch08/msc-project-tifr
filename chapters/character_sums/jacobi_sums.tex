...
\begin{Theorem}{Fermat's Two-Square Theorem}{thm:fermat-two-square}
  Let $p$ be an odd prime with $p\equiv 1\bmod 4$, let $\eta$ denote the quadratic character of $\bbF_p$, and let $\lm\in\Mp$ be a multiplicative character of $\bbF_p$ of order $4$. Then there exist integers $A,B\in\bbZ$ such that $$J(\lm,\eta)=A+iB\qquad\text{and}\qquad p=A^2+B^2.$$
\end{Theorem}
\begin{proof}
...
Since for the trivial multiplicative character $\psi_0\in\Mq$ we have $\lm^0=\psi_0$. Therefore $\prod_{j=0}^{m-1}G(\lm^j,\chi)=G(\psi_0,\chi)\prod_{j=1}^{m-1}G(\lm^j,\chi)=-\prod_{j=1}^{m-1}G(\lm^j,\chi)$ by \GSpropthm. Therefore from \eqref{eq:hs-relation-wiki} we have $$\prod_{j=0}^{m-1}G(\psi\cdot \lm^j)=-G(\psi^m,\chi_m)\prod_{j=0}^{m-1}G(\lm^j,\chi)$$ So we get the following observation:
\begin{observation}
