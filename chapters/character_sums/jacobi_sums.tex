\section{Jacobi Sums}\label{sec:jacobi-sums}
\index{Jacobi sum}\index{Jacobi, Carl Gustav Jacob}%
...
\begin{observation}\label{obs:jacobi-sum-symmetric}
...
\subsection{Evaluating Jacobi Sums}
...
\begin{lemma}{Jacobi Sums with Trivial Characters}{lem:jacobi-some-trivial}
\index{Jacobi sum!degenerate}%
...
\begin{lemma}{Relation between $J_0$ and $J$}{lem:jacobi-zero-relation}
...
\begin{Theorem}{Jacobi--Gaussian Sum Relation}{thm:jacobi-gaussian}
...
\begin{Theorem}{Absolute Value of Non-trivial Jacobi Sums}{thm:jacobi-sum}
...
\begin{corolary}{Absolute Value of $J_0$ for Non-trivial Characters}{cor:jacobi-J0-abs}
...
\begin{Theorem}{Complete Evaluation of Jacobi Sums}{thm:jacobi-sum-complete}
  \index{Jacobi sum!evaluation}%
...
\subsection{The Davenport--Hasse Relations}
...
\begin{lemma}{Gaussian Sum Power via Jacobi Products}{lem:gauss-power-jacobi}
...
\begin{Theorem}{Davenport--Hasse Product Formula}{thm:davenport-hasse-product}%
  \index{Davenport-Hasse theorem!product formula}
...
\begin{Theorem}{Davenport--Hasse Relation over $\bbF_p$, \cite[Theorem~3]{HuardSpearmanWilliams_1995_AAA}}{thm:davenport-hasse-relation}%
  \index{Davenport-Hasse theorem!relation over $\mathbb{F}_p$}
...
\begin{corolary}{Product of Twisted Gaussian Sums}{cor:gauss-product-twisted}
...
\begin{corolary}{}{cor:hs-relation-product}
   Let $\psi,\lm\in\Mq$ be multiplicative characters of $\bbF_q$ such that $\ord(\lm)=m\geq 2$ and $\psi^m$ is non-trivial. Let $\chi\in\Xq$ be any non-trivial additive character of $\bbF_q$. Then $$\prod_{j=0}^{m-1}G(\psi\cdot \lm^j)=\psi^{-m}(m)\cdot G(\psi^m,\chi)\prod_{j=1}^{m-1}G(\lm^j,\chi)$$where for any $\gm\in\bbF_q$, $\chi_m(\gm)=\chi(m\cdot \gm)$ similar to as defined in \AddCharDefthm.
\end{corolary}
\begin{proof}
  By \DHprodthm we have $$\frac{G(\psi,\chi)^m}{G(\psi^m,\chi)}=\prod_{j=1}^mJ(\psi,\lm^j)$$Now  using \JGrelthm for any $j\in\{m-1\}$ we have $$J(\psi,\lm^j)=\frac{G(\psi,\chi)\cdot G(\lm^j,\chi}{G(\psi\cdot\lm^j,\chi)}$$So replacing $J(\psi,\lm^j)$ we get
  $$ \frac{G(\psi,\chi)^m}{G(\psi^m,\chi)}  = \prod_{j=1}^{m-1}\frac{G(\psi,\chi)\cdot G(\lm^j,\chi}{G(\psi\cdot\lm^j,\chi)}=G(\psi,\chi)^{m-1}\prod_{j=1}^{m-1}\frac{ G(\lm^j,\chi}{G(\psi\cdot\lm^j,\chi)}$$So we get the final relation \begin{equation}\label{eq:hs-relation-wiki}
    \prod_{j=0}^{m-1}G(\psi\cdot \lm^j)=G(\psi^m,\chi_m)\prod_{j=1}^{m-1}G(\lm^j,\chi)
  \end{equation}
  Now by \GSidentitieslem[(i)] we have $G(\psi^m,\chi_m)=\psi^{-m}(m)G(\psi^m,\chi)$. So replacing $G(\psi^m,\chi_m)$ we get the theorem. 
\end{proof}

Since for the trivial multiplicative character $\psi_0\in\Mq$ we have $\lm^0=\psi_0$. Therefore $\prod_{j=0}^{m-1}G(\lm^j,\chi)=-\prod_{j=1}^{m-1}G(\lm^j,\chi)$ by \GSpropthm. Therefore from \eqref{eq:hs-relation-wiki} we have $$\prod_{j=0}^{m-1}G(\psi\cdot \lm^j)=-G(\psi^m,\chi_m)\prod_{j=0}^{m-1}G(\lm^j,\chi)$$ So we get the following observation:
\begin{observation}\label{obs:gauss-product-sign}
  Let $\psi,\lm\in\Mq$ be multiplicative characters of $\bbF_q$ such that $\ord(\lm)=m\geq 2$ and $\psi^m$ is non-trivial. Let $\chi\in\Xq$ be any non-trivial additive character of $\bbF_q$. Then $$\prod_{j=0}^{m-1}G(\psi\cdot \lm^j)=-\psi^{-m}(m)\cdot G(\psi^m,\chi)\prod_{j=0}^{m-1}G(\lm^j,\chi)$$
\end{observation}
\subsection{Application: Fermat's Two-Square Theorem}

A classical application of the arithmetic of Jacobi sums is a short representation-theoretic proof of a celebrated theorem of Fermat: every prime $p\equiv 1\bmod 4$ is a sum of two integer squares. The key point is that when $\lm$ is a multiplicative character of $\bbF_p$ of order $4$, the Jacobi sum $J(\lm,\eta)$ lies in the ring of Gaussian integers $\bbZ[i]$ and has absolute value exactly $\sqrt p$.
\begin{Theorem}{Fermat's Two-Square Theorem}{thm:fermat-two-square}%
  \index{Fermat's two-square theorem}
  Let $p$ be an odd prime with $p\equiv 1\bmod 4$. Then there exist integers $A,B\in\bbZ$ such that $p=A^2+B^2$.
\end{Theorem}
\begin{proof}
  Let $\lm,\eta\in\Mq$ are multiplicative characters such that $\ord(\lm)=4$ and $\eta$ is the quadratic character. Since $\ord(\lm)=4$, $\lm$ takes values in $\{\pm 1,\pm i\}\subseteq \bbZ[i]$, and since $\eta$ takes values in $\{\pm 1\}\subseteq \bbZ$, the Jacobi sum $$J(\lm,\eta)=\sum_{\alpha\in\bbF_p}\lm(\alpha)\cdot \eta(1-\alpha)\in\bbZ[i].$$Write $J(\lm,\eta)=A+iB$ with $A,B\in\bbZ$. Since $\lm$ and $\eta$ are both non-trivial and $\lm\cdot \eta=\lm^3$ is also non-trivial (as $\ord(\lm)=4$), \thmref{thm:jacobi-sum} gives $|J(\lm,\eta)|=p^{\nicefrac12}$; equivalently $A^2+B^2=p$.
\end{proof}

We give another proof of this theorem in \autoref{sec:fermat-2-square-another} during the discussion of Jacobsthal Sums in \autoref{sec:jacobsthal}. 