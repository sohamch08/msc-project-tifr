\section{Jacobsthal Sums}\label{sec:jacobsthal}
...
\begin{lemma}{Relation between $H_n$ and $I_m$}{lem:jacobsthal-Iduplication}
...
\begin{Theorem}{Jacobi Sum Representation of $I_n$}{thm:jacobsthal-In-jacobi}
...
\begin{lemma}{Jacobi Sum Representation of $H_n$}{thm:jacobsthal-Hn-jacobi}
...
\end{proof}
\begin{note}
  By \JacobiCompletethm, $|J(\lm^j,\eta)|=q^{1/2}$ for each $j\in\{1,\dots,d-1\}$ since $\lm^j$ and $\eta$ are non-trivial and $\lm^j\eta$ is non-trivial for $j\neq d/2$. Summing the $d-1$ terms in \thmref{thm:jacobsthal-In-jacobi} therefore gives $|I_n(\alpha)|\leq (d-1)q^{\nicefrac12}$ for all $\alpha\in\bbF_q^*$.
\end{note} 

\subsection{Yet Another Proof of Fermat's Two Square Theorem}\label{sec:fermat-2-square-another}
