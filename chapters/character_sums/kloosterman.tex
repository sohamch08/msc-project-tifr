\section{Kloosterman Sums}\label{sec:kloosterman}
We now turn to a genuinely different family of exponential sums, introduced by Hendrik Kloosterman\index{Kloosterman, Hendrik} in 1926 in his refinement of the Hardy--Littlewood circle method\index{circle method} for the representation of integers by positive definite quaternary quadratic forms. Instead of twisting two characters of different types, a Kloosterman sum twists a single additive character $\chi$ with the \emph{inversion} map\index{inversion map} $\gm\mapsto \gm^{-1}$ on $\bbF_q^*$, producing a sum of the form $\sum_{\gm}\chi(a\gm+b\gm^{-1})$.

\begin{Definition}{Kloosterman Sums}{def:kloosterman}
  \index{Kloosterman sum}%
...
When one or both of the parameters $a,b$ vanish, the Kloosterman sum collapses to an additive character sum on $\bbF_q^*$, which we have already computed in \AddCharPropsthm.\index{Kloosterman sum!degenerate} If both $a=b=0$ then for all $\gm\in\bbF_q^*$, $\chi(a\cdot \gm+b\cdot \gm^{-1})=\chi(0)=1$. Hence $K(\chi;0,0)=q-1$. 
...
Now another property to notice is Kloosterman sum is always real\index{Kloosterman sum!real-valued} since $$\ov{K(\chi;a,b)}=\sum_{\gm\in\bbF_q^*}\ov{\chi}(a\cdot \gm+b\cdot \gm^{-1})= \sum_{\gm\in\bbF_q^*}\chi(-(a\cdot \gm+b\cdot \gm^{-1}))=\sum_{\gm\in\bbF_q^*}\chi(a\cdot (-\gm)+b\cdot (-\gm)^{-1})=K(\chi;a,b)$$
...
The following theorem is the Kloosterman analogue of the \DHthm: the entire sequence of lifted sums $K(\chi^{(s)};a,b)$ is controlled by just two algebraic numbers $w_1,w_2$ --- the reciprocal roots of the associated $L$-function. The proof uses the Dirichlet $L$-function theory from \autoref{sec:l-functions}.
\begin{Theorem}{Lifting of Kloosterman Sum}{thm:kloosterman-lift}
  \index{Weil bound!Kloosterman}%
...
\begin{proof}
  ...
  \begin{align*}
    \sum_{g\in\Phi_k}\Lambda(g) & = \sum_{c_1,\dots, c_{k-1}}\sum_{c_k\in\bbF_q^*}\chi(a\cdot c_1+b\cdot c_{k-1}\cdot c_k^{-1})\\ 
  ...
  \end{align*} Hence we have $\ovL(z,\Lambda)=1+z\sum_{h\in\Phi_1}\Lambda(h)+z^2\sum_{h\in\Phi_2}\Lambda(h)$. Now $\Phi_1=\{X-\gm\mid \gm\in\bbF_q^*\}$. Therefore we have $$\sum_{h\in\Phi_1}\Lambda(h)=\sum_{\gm\in\bbF_q^*}\chi(a\cdot \gm+b\cdot \gm^{-1})=K(\chi;a,b)$$And furthermore for $\Phi_2$ we get \begin{align*}
    \sum_{h\in\Phi_2}\Lambda(h) & = \sum_{c_1\in\bbF_q}\sum_{c_2\in\bbF_q^*}\Lambda(X^2-c_1\cdot X+c_2)\\ 
  ...
  \end{align*}
  So if $c_2=-a^{-1}b$ then $a+b\cdot c_2^{-1}=0$ and hence $\sum_{c_1\in\bbF_q}\chi(0)=q$ and if $c_2\neq -a^{-1}b$ then $\sum_{c_1\in\bbF_q}\chi(c_1(a+b\cdot c_2^{-1}))=0$ by \AddCharPropsthm[(i)]. So therefore $\sum_{h\in\Phi_2}\Lambda(h)=q$ and thus we get $\ovL(z,\Lambda)=1+k(\chi;a,b)\cdot z+q\cdot z^2$. Let $w_1,w_2\in\bbC$ which are either complex conjugates or both real such that $$\ovL(z,\Lambda)=(1-w_1\cdot z)(1-w_2\cdot z)$$
...
\begin{lemma}{Reduction of Lifted Kloosterman Sums}{lem:kloosterman-reduction}
...
\begin{corolary}{Kloosterman Sum Recurrence Relation}{cor:kloosterman-recursion}
