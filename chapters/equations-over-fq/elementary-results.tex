...
\begin{note}
The bound $\deg(f)<n$ in \CWthm is the best possible: the norm polynomial $f(X_1,\dots, X_n)=\prod_{j=0}^{n-1}\lt(\sum_{i=1}^n \alpha_i^{q^j}X_i\rt)$, where $\{\alpha_1,\dots, \alpha_n\}$ is a basis of $\bbF_{q^n}$ over $\bbF_q$, has degree $n$ and satisfies $f(\gm_1,\dots,\gm_n)=N_{\bbF_{q^n}/\bbF_q}(\sum_i\alpha_i\gm_i)$, which is zero only when $\gm_j=0$ for all $j$. Thus $Z(f)=1$, so divisibility by $p\geq 2$ fails.
\end{note} 
...
We can give another upper bound based on individual degrees of each variables. If degree of each variable is at most $d$ then there can be at most $d(q^{n-1}-1)$ many non-trivial solutions. For this we consider $f$ to be \emph{homogeneous}\index{homogeneous polynomial} i.e. total degree of each monomial is same.
