\section{Some Elementary Results}\label{sec:elementary-results}
...
We begin with two elementary lemmas about power sums\index{power sum} that will serve as the key tools.
...
\end{lemma}
\begin{proof}
  For $k=0$ the sum becomes $q\equiv 0 \bmod q$. So suppose $k>0$. Let $\gm$ be the primitive element of $\bbF_q$. Then we have $$\sum_{\alpha\in\bbF_q}\alpha^k=\sum_{\alpha\in\bbF_q^*}\alpha^k=\sum_{j=0}^{q-2}\gm^{j\cdot k}=\sum_{j=0}^{q-2}(\gm^k)^j$$Now if $q-1\mid k$ then $\gm^k=1$ so the sum evaluates to be $-1$. If $q-1\nmid k$ then $\sum_{j=0}^{q-2}(\gm^k)^j=\frac{\gm^{k(q-1)}-1}{\gm^k-1}=0$. Hence we have the lemma. 
\end{proof}
...
\begin{Theorem}{Chevalley--Warning Theorem}{thm:chevalley-warning}
  \index{Chevalley--Warning theorem}\index{Chevalley, Claude}\index{Warning, Ewald}%
...
\begin{note}
The bound $\deg(f)<n$ in \CWthm is the best possible: the norm polynomial\index{norm!polynomial} $f(X_1,\dots, X_n)=\prod_{j=0}^{n-1}\lt(\sum_{i=1}^n \alpha_i^{q^j}X_i\rt)$, where $\{\alpha_1,\dots, \alpha_n\}$ is a basis of $\bbF_{q^n}$ over $\bbF_q$, has degree $n$ and satisfies $f(\gm_1,\dots,\gm_n)=N_{\bbF_{q^n}/\bbF_q}(\sum_i\alpha_i\gm_i)$, which is zero only when $\gm_j=0$ for all $j$. Thus $Z(f)=1$, so divisibility by $p\geq 2$ fails.
\end{note} 
...
\begin{Theorem}{Chevalley--Warning Theorem for Systems of Equations}{thm:cw-systems}
...
\begin{Theorem}{Lower Bound via Parallel Shifts}{thm:lower-bound-parallel-shifts}
  \index{Warning's bound}%
...
We now give upper bounds on the number of solutions of a polynomial equation. The first is the celebrated Schwartz--Zippel lemma,\index{Schwartz--Zippel lemma} which bounds the probability of a random point being a root in terms of total degree.
\begin{lemma}{Schwartz-Zippel Lemma}{lem:schwartz-zippel}
  \index{Schwartz-Zippel lemma}%
...
\begin{corolary}{Total Degree Solution Bound}{}
...
We can give another upper bound based on individual degrees of each variables. If degree of each variable is at most $d$ then there can be at most $d(q^{n-1}-1)$ many non-trivial solutions. For this we consider $f$ to be \emph{homogeneous}\index{homogeneous polynomial}\index{polynomial!homogeneous} i.e. total degree of each monomial is same.
\begin{Theorem}{Nontrivial Solution Bound for Homogeneous Polynomials}{thm:homogeneous-bound}
