...
Let $G$ be the group of all rational functions $r(X)\in\bbF_q(X)$ such that for any $r(X)=\nicefrac{h_1(X)}{h_2(X)}$ where $h_1,h_2\in\Phi$. Let $f(X)\in\bbF_q[X]$ be a monic polynomial in $\bbF_q[X]$. Let $f(X)$ factors into $$f(X)=(X-\gm_1)^{a_1}\cdots (X-\gm_m)^{a_m}$$ in the field $\ov{\bbF}_q[X]$ i.e. $f$ has $m$ distinct roots. Then define the subgroup $\ov{G}$ such that $r=\nicefrac{h_1}{h_2}\in\ovG$ then $$h_1(\gm_i)\cdot h_2(\gm_i)\neq 0\quad\forall\ i\in[m]$$Then $\ovG$ has the property that if $r_1\cdot r_2\in\ovG$ then $r_1,r_2\in \ovG$. 
