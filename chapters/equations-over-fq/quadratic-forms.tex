...
To calculate and understand the of solutions of quadratic forms we define the integer-valued function $\vth:\bbF_q\to\bbZ$ by $\vth(\alpha)=-1$ for all $\alpha\in\bbF_q^*$ and $\vth(0)=q-1$. Then we have the following properties of $\vth$ function. 
...
We now resume the study of quadratic forms $f\in\bbF_q[X_1,\dots,X_n]$ where $q$ is odd. We will first show that every quadratic form $f\in\bbF_q[X_1,\dots,X_n]$ where $q$ is odd is equivalent to a \emph{diagonal quadratic form} i.e. a quadratic form of type $a_1X_q^2+\cdots+a_nX_n^2$ where $a_i\in\bbF_q$ for all $i\in[n]$. 
...
Now it is easy to deduce that from any general quadratic form $f\in\bbF_q[X_1,\dots,X_n]$ we can separate all the variables one by one and get an equivalent diagonal quadratic form. But there are some tricky cases to remain since we also have to satisfy that $f$ or its equivalent form represents a non-zero element of $\bbF_q$. 
...
\end{observation}
If the coefficient matrix $C_f$ for any quadratic form $f\in\bbF_q[X_1,\dots,X_n]$ has rank $n$ then we call $f$ to be \emph{non-degenerate}. We also define \emph{determinant of $f$}, $\det(f)$ to be the determinant of the coefficient matrix $C_f$. 

Now we will study number of solutions of $2$-variable quadratic diagonal forms which we will later use in \QFDiagthm to calculate the number of solutions of general diagonal forms. 
...
We will now present alternate proof of this theorem using Jacobi sums. This will help us how to use the machinery built in previous chapter properly to get values for number of solutions. 
...
For even characteristic fields we define \emph{non-degenerate} quadratic forms differently. Let $f\in\bbF_q[X_1,\dots,X_n]$ is a quadratic form in $n$-variables. Then $f$ is called non-degenerate if $f$ is not equivalent to a quadratic form in fewer than $n$ variables. Notice this definition is also valid for the definition of \emph{non-degenerate} in odd characteristic case. Since there if $f$ is equivalent to a quadratic form with fewer than $n$ variables then that quadratic form is equivalent to a diagonal quadratic form with fewer than $n$ variables which means by \QFDiagRankDetObs that $f$ is not non-degenerate. So it suffices to discuss when $f$ is non-degenerate otherwise we continue to go to the equivalent quadratic form with fewer variables till that form becomes a non-degenerate quadratic form.  
...
\end{proof}
So now we will use this theorem to find proper canonical forms for any general quadratic forms. 
\begin{Theorem}{Canonical Forms for even characteristic}{thm:qf-canonical-even-char}
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item If $n$ is even then $f$ is \begin{enumerate}
      ...
      \item or to $X_1X_2+\cdots+X_{n-3}X_{n-2}+X_{n-1}^2+a\cdot X_n^2$ where $a\in\bbF_q$ satisfies $\tr(a)=1$.
    \end{enumerate}
  \end{enumerate}
\end{Theorem}
\begin{proof}
...
\begin{Theorem}{Solution Counts for Canonical Quadratic Forms in even characteristic}{thm:qf-count-even-char}
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \begin{enumerate}[label=(\alph*)]
      ...
      \item for $a\in\bbF_q$ with $\tr(a)=1$, then $Z\lt(X_1X_2+\cdots+X_{n-3}X_{n-2}+X_{n-1}^2+a\cdot X_n^2=\alpha\rt)=q^{n-1}-\vth(\alpha)\cdot q^{\nicefrac{(n-2)}{2}}$.
    \end{enumerate}
  \end{enumerate}
\end{Theorem}
\begin{proof}
