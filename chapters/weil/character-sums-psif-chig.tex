...
\begin{corolary}{Reciprocal Roots of the Lifted $L$-function}{cor:lifted-L-roots}
  If $L(s,\xi)$ is given by $L(s,\xi)=(1-w_1u)\cdots (1-w_lu)$ with $u=q^{-s}$ then $$ L^{(k)}\lt(s,\xi^{(k)}\rt)=(1-w_1^k\cdot u_k)\cdots (1-w_l^k\cdot u_k)$$where $u_k=q^{-ks}$.
\end{corolary}

\begin{corolary}{Character Sum via Reciprocal Roots}{cor:charsum-reciprocal-roots}
  Suppose either $\psi\in\Mq$ is a nontrivial multiplicative character of exponent $d$ and $Y^d-f(X)$ is absolutely irreducible. If $\chi\in\Xq$ is a additive character either non-trivial or trivial and $f(X)=1$ then $$S^{(k)}=-\sum_{i=1}^{n+m-1}w_i^k.$$
\end{corolary}
