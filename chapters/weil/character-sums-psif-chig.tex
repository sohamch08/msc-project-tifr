\section{Characters sums of \texorpdfstring{$\psi(f(X))\chi(g(X))$}{Ψ(f(X))χ(g(X))}}\label{sec:character-sums-psif-chig}
...
We want to create the $L$-function on $\xi$ and use it to calculate the character sums and get bounds on absolute value of the character sum. We will show that the $L$-function $\ovL(z,\xi)$ is of finite degree. First we will show $\xi$ is indeed non-trivial. 
...
\begin{Theorem}{Finite Degree of $\ovL(z,\xi)$}{thm:ovL-xi-finite}
  \index{L-function!finite degree}%
  Suppose either $\psi\in\Mq$ is a nontrivial multiplicative character of exponent $d$ and $Y^d-f(X)$ is absolutely irreducible. Or, $\chi\in\Xq$ is a non-trivial additive character and
  \begin{enumerate}[label=(\roman*)]
    \item either, $\gcd(n,q)=1$
    \item or, more generally $Y^q-Y-g(X)$ is absolutely irreducible.
...
So now we can give an analog of \LLeadCoeffthm for the lifted sum and give a relation with $\Wmixl{k}{f}{g}$ with the coefficients of $\Lxil{k}$. 
...
\index{Davenport-Hasse relation!alternate proof}%