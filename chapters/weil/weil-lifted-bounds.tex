...
In this section we will try to give bounds on the absolute value of deviation of number of zeros from the average number of zeros, $q^{n-1}$. And then we will give upper bounds on the absolute values of $\Wpsil{k}{f}$, $\Wchil{k}{g}$ and $\Wmixl{k}{f}{g}$. 

\subsection{Bound for Multiplicative Character Sums}
Let $\psi\in\Mq$ be a non-trivial multiplicative character. Let $f(X)\in\bbF_q[X]$ is a monic polynomial with precisely $m$ distinct roots. Then $$\Wpsi{f}=\sum_{\alpha\in\bbF_q}\psi(f(\alpha)),\qquad \Wpsil{k}{f}=\sum_{\alpha\in\bbF_{q^k}}\psi^{(k)}(f(\alpha))$$So by \LiftedLRootscor there exists complex numbers $w_1,\dots, w_{m-1}\in\bbC$ such that $$\Wpsil{k}{f}=\sum_{\alpha\in\bbF_{q^k}}\psi^{(k)}(f(\alpha))=-\sum_{j=1}^{m-1}w_i^k$$Suppose $\psi\in\Mq^{(d)}$ where $d>1$. Then there are $d$ characters of exponent $d$. For each such character $\psi$ we define the sums 
